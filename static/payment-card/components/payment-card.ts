...
import { getGiftCardOrderId } from "../../../functions/helpers/shared";
...
export async function presentPaymentCard(contentElement: HTMLElement) {
  ...
  const suitableCard = dummyCardSandbox as unknown as Card; //await getSuitableCard(cards, countryCode, reward);
...
export function getSingleGiftCardHtml(card: Card, amount: BigNumberish): string {
  return html`
    <div class="summary">
...
function addCardEvents(card: Card) {
